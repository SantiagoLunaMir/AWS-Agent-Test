...
`cdk-outputs.json` contiene `ChatUrl`, `PanelUrl`, `ApiUrl`, `ChatKeyCommand` y `PanelKeyCommand`. Está en `.gitignore`: **no lo subas**. Para que un humano pruebe el chat, dale los comandos de la sección "Enlaces del chat y del panel" del README: imprimen `<ChatUrl>/#clave=<código>` y `<ChatUrl>/panel.html#clave=<clave>`. En el panel, el botón **Compartir chat** muestra el mismo enlace con un QR.
