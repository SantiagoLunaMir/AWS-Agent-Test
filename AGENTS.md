...
- `backend/taller/`: Lambdas en Python 3.12, con Claude en Amazon Bedrock mediante el SDK oficial `anthropic` (`AnthropicBedrock`).
...
# 3. ¿Qué modelo Claude puede invocar la cuenta? (usa el que recomiende)
python scripts/probar_modelos.py --region us-east-2

# 4. Desplegar (bootstrap solo la primera vez por cuenta y región)
cd infra
npx -y aws-cdk@latest bootstrap
npx -y aws-cdk@latest deploy --require-approval never --outputs-file ../cdk-outputs.json -c modelId=<modelo del paso 3>
cd ..
...
|---|---|---|
| `404 Model use case details have not been submitted` | La cuenta no envió el formulario de Anthropic en Bedrock. El acceso puede ser **intermitente** mientras se procesa | Un humano debe completarlo en la consola de Bedrock (*Model catalog* → modelo de Anthropic → *Submit use case details*). No lo hagas tú: pide datos de la empresa |
| `403 ... is not available for this account` | Ese modelo no está habilitado en la cuenta | Usa el que recomiende `scripts/probar_modelos.py` |
| `400` sobre `thinking` o `effort` | Modelo anterior a la familia 4.6 | El código requiere Opus/Sonnet 4.6 o más nuevos |
...
Cambia `Agente` por `Api` o `Recordatorio` para ver las otras funciones. Cada llamada a una herramienta se registra como `tool <nombre>(<entrada>) -> error=<bool>`.
...
- **Herramientas:** `backend/taller/herramientas.py`. Las reglas críticas (confirmación del cliente, horarios, máximo 2 citas activas, reserva atómica del horario, dueño de la cita) se validan **en código**. Si agregas una herramienta, valida ahí también, no solo en el prompt.
- **Foto opcional:** el cliente puede dar marca y modelo por texto. Si manda foto, `vision.py` la clasifica en una llamada aislada con salida JSON por esquema. La imagen **nunca** entra a la conversación del agente.
...
- **Configuración:** variables de entorno en `backend/taller/config.py`. Contexto de CDK: `modelId`, `modoRecordatorio` (`demo` = 2 min después de agendar; `real` = 24 h antes) y `senderEmail` (SES opcional; en sandbox el destinatario debe estar verificado).
